 ...
 function loadTooltipChart(id, year, genre, studio){
		...
		if(!studio){
		 ...
		 var svg = dimple.newSvg("#" + id, chartWidth, chartHeight);
			...
			if ( data.length == 0 ){
			  ...
			  var popup = null;
			  myChart.setBounds(60, 50, chartWidth - 160, chartHeight - 150);
			  ...
			  function onHover(e) {
				...
				// Create a new mini chart of Sales over Months
				tipChart = new dimple.chart(svg,  data);
				tipChart.setBounds(x + 10, y + 50, width - 10, height - 40);
				tipChart.addCategoryAxis("x", "date").hidden = true;
				tipChart.addMeasureAxis("y", "expectedearning").hidden = true;
	...
	var dynamicwidth=$("#linechart").width();
		if(id == undefined)return;
		var width =dynamicwidth, height = 300;
		...
		var svg = dimple.newSvg("#" + id, width, height);
			d3.tsv("data/line-chart-data.tsv", function (data) {
			  ...
			  var myChart = new dimple.chart(svg, data);
			  myChart.setBounds(60, 30, width-30, height-100);
	...
	function loadHeatMap(id){
	 $('#'+id).append('<span id="loading">Loading....</span>');
		...
		setTimeout(function(){
			...
			if($(document).width() <= 768){
				$('#'+id).append('<img src="img/trends.jpg" id="trendsImg" width="400">');
			} else {
				$('#'+id).append('<img src="img/trends.jpg" id="trendsImg">');
			}
